...
def analyze_and_build_figure(
    ...
    # ---- 1) Compute SSQ-CWT ----
    # Tx = SSWT (用於 Ridge 提取，因為比較精細)
    # Wx = CWT (用於左圖背景顯示，因為比較平滑)
    try:
    ...
    mag_sswt = np.abs(Tx)
    mag_cwt = np.abs(Wx)  # 這是左圖要畫的能量

    # 計算 SSWT 的週期 (給 Ridge 用)
    with np.errstate(divide="ignore", invalid="ignore"):
        periods_sswt = 1.0 / ssq_freqs

    # 計算 CWT 的週期 (給左圖 Y 軸用)
    # 我們利用 scales 轉換回物理週期
    w_obj = Wavelet(wavelet)
    wc = float(getattr(w_obj, "wc", 2 * np.pi))
    # Frequency = (wc * fs) / (2 * pi * scales)
    # Period = 1 / Frequency
    freqs_cwt = (wc * float(fps)) / (2 * np.pi * np.maximum(scales, 1e-12))
    periods_cwt = 1.0 / np.maximum(freqs_cwt, 1e-12)
    ...
    # ---- 3) Ridge Extraction (完全保留您原本的邏輯) ----
    # 這裡我們使用 SSWT (mag_sswt) 進行計算，因為它的頻率解析度比 CWT 更好，抓 Ridge 更準
    ...
    # ---- 4) Build Figure (關鍵修改處：顏色與座標軸) ----
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=("(a) CWT Scalogram (Morlet)", "(b) Ridge Extraction"),
        horizontal_spacing=0.18,
    )

    # === 左圖：CWT (修正顏色與座標) ===
    # 1. 使用原始 CWT 振幅 (mag_cwt)
    # 2. Y軸使用計算好的 periods_cwt (確保是週期)
    # 3. 色階使用 'Jet' (讓強能量顯示為紅色)
    fig.add_trace(
        go.Heatmap(
            z=mag_cwt,
            x=time_axis,
            y=periods_cwt,
            colorscale="Jet",  # ✅ 這裡改成 Jet，就會有彩虹色（紅=強，藍=弱）
            colorbar=dict(
                title="Magnitude",
                thickness=14,
                len=0.82,
    ...
    # === 右圖：Ridge Extraction (保持原樣，僅調整點的顏色) ===
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    ...
                    colorscale="Jet", # ✅ 右邊的點點圖也改成 Jet，風格一致
                    showscale=False,
    ...
    if transition_events:
        ...
        fig.add_annotation(
            x=t0, y=np.log10(y_max) if y_max > 0 else 1, # 注意：因為Y軸是log，這裡標註位置要小心
            xref="x2", yref="y2",
            text="Transition",
            showarrow=False,
            yshift=10,
    ...
    # ✅ 設定範圍 (使用 Log 座標顯示週期)
    # 因為 Scale 是線性的，但 Period 是倒數關係，所以通常畫 Period 都用 Log 軸比較好看
    y_range_log = [np.log10(y_min), np.log10(y_max)]
    ...
    # 左圖 Y 軸：Period (s)
    fig.update_yaxes(title_text="Period (s)", type="log", range=y_range_log, **axis_settings, row=1, col=1)
    # 右圖 Y 軸：Period (s)
